...
async def create_controller_agent(browser_agent: Agent, doc_processor_agent: Agent):
    ...
    controller_agent = Agent(
        name="WorkflowController", # 代理名称
        # agent 指令
        instructions="""
        你是GitHub技术文档检索和处理系统的主控制器。你负责协调整个工作流程，确保从GitHub检索文档
        ...
        handoffs=[doc_processor_agent], # 文档处理 agent 作为交接选项提供给控制器 agent, 参考：https://openai.github.io/openai-agents-python/handoffs/
        model=model_provider.get_model(MODEL_NAME), # 使用我们自定义的模型提供商获,确保agent使用正确的模型
        model_settings=ModelSettings(
...
async def process_user_query(query: str):
    ...
    Returns:
        bool: 处理才成功返回True，否则返回False，用于向用户反馈操作结果
    """
    ...
    try:
        ...
        # 简化查询指令，专注于查询内容
        formatted_query = f"""
        请在Github上查找关于"{query}"的技术文档，并翻译成中文
        """
        ...
        print("\n========== 任务完成 ==========")
        if hasattr(result, "fianl_output"):
            print("\n工作流结果：")
            print(result.fianl_output)
